...
public class DeleteUserRequest extends AbstractRequest {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        try {
            ...
            if (user == null) {
                NetUtility.sendResponse(exchange, new DeleteUserResponse("未找到用户：" + username));//一般情况下不会出现这种情况
            } else if (deletedUser == null) {
                NetUtility.sendResponse(exchange, new DeleteUserResponse("未找到用户：" + to_delete));
            } else if (user.getLevel() < User.OPERATOR) {//用户权限不足，不能删人
                NetUtility.sendResponse(exchange, new DeleteUserResponse("权限不足！"));
            } else if (user.getLevel() <= deletedUser.getLevel()) {//只能上删下，不能下删上
                NetUtility.sendResponse(exchange, new DeleteUserResponse(
                        String.format("%s(等级%d)的权限等级不低于您，不能删除它的信息", to_delete, deletedUser.getLevel())));
            } else {
                ...
                if (result == 1) {
                    NetUtility.sendResponse(exchange, new DeleteUserResponse("删除成功！"));
                } else {
                    NetUtility.sendResponse(exchange, new DeleteUserResponse("删除失败！"));
                }
                statement.close();
                connection.close();
            }
        } catch (SQLException e) {
            NetUtility.sendResponse(exchange, new DeleteUserResponse("未知错误：" + e.getMessage()));
